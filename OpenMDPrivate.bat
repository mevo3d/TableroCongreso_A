...
start "" %EDGE_PATH% --inprivate --new-window "%BASE_URL%/autologin/operador"
timeout /t 2 /nobreak >nul

echo [2/6] Abriendo SECRETARIO LEGISLATIVO...
start "" %EDGE_PATH% --inprivate "%BASE_URL%/autologin/secretario"
timeout /t 2 /nobreak >nul
...
start "" %EDGE_PATH% --inprivate "%BASE_URL%/autologin/isaac.pimentel"
timeout /t 2 /nobreak >nul

echo [4/6] Abriendo VICEPRESIDENTE - Eleonor Martinez Gomez...
start "" %EDGE_PATH% --inprivate "%BASE_URL%/autologin/eleonor.martinez"
timeout /t 2 /nobreak >nul

echo [5/6] Abriendo SECRETARIO 1 - Guillermina Maya Rendon...
start "" %EDGE_PATH% --inprivate "%BASE_URL%/autologin/guillermina.maya"
timeout /t 2 /nobreak >nul

echo [6/6] Abriendo SECRETARIO 2 - Alberto Sanchez Ortega...
start "" %EDGE_PATH% --inprivate "%BASE_URL%/autologin/alberto.sanchez"
timeout /t 2 /nobreak >nul
...
echo.
echo Total: 6 ventanas abiertas en Microsoft Edge (modo InPrivate)
echo.
